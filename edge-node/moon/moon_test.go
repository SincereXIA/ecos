...
func TestRaft(t *testing.T) {
	...
	time.Sleep(2 * time.Second) // wait for clusterInfoStorage apply
	...
	// 判断集群是否达成共识
	assertInfoStorageOK(t, len(moons), moons...)
	info := moons[0].clusterInfoStorage.ListAllNodeInfo()
	t.Log(info)
	for i := 1; i < 4; i++ {
		anotherInfo := moons[i].clusterInfoStorage.ListAllNodeInfo()
...
func assertInfoStorageOK(t *testing.T, nodeNum int, moons ...*Moon) {
	firstClusterInfo := moons[0].clusterInfoStorage.GetClusterInfo(0)
	for _, moon := range moons {
		storage := moon.clusterInfoStorage
...
func waitMoonsOK(moons []*Moon) int {
	...
	for {
		...
		for i := 0; i < len(moons); i++ {
			if moons[i].GetLeaderID() == 0 || len(moons[i].clusterInfoStorage.ListAllNodeInfo()) != len(moons) {
				ok = false
			}
