...
func (r *EcosReader) Read(p []byte) (n int, err error) {
	...
	for i := 0; i < blockNum && r.curBlockIndex < len(r.meta.Blocks); i++ {
		...
		blockInfo := r.meta.Blocks[r.curBlockIndex]
		if uint64(end-start+offset) == r.config.Object.BlockSize {
			r.curBlockIndex++
		}
		waitGroup.Add(1)
		go func(info *object.BlockInfo, start int, end int, offset int) {
			...
			if end-start+offset < len(block) {
				...
				isEOF = false
			} else if offset > 0 { // 当前 block 不是从头读的，但已经读完，需要重置 offset，否则下一个 block 不会从头读
				r.curBlockOffset = 0
			}
			waitGroup.Done()
	...
	if r.curBlockIndex >= len(r.meta.Blocks) || (r.curBlockIndex == len(r.meta.Blocks)-1 && isEOF) {
		r.curBlockIndex = len(r.meta.Blocks)
